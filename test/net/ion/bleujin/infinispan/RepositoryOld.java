...
public class RepositoryOld implements Repository{
	
	private IExecutor executor = new IExecutor(0, 3) ;
	private Map<String, AbstractWorkspace> wss = MapUtil.newCaseInsensitiveMap() ;
	private Map<String, OldCacheStoreConfig> configs = MapUtil.newCaseInsensitiveMap() ;
	...
	public RepositoryOld(DefaultCacheManager dm){
		this.dm = dm ;
		putAttribute(ColumnParser.class.getCanonicalName(), new ColumnParser()) ;
	}
	
	public static RepositoryOld create() {
	...
	public static RepositoryOld create(GlobalConfiguration gconfig) {
		...
		return new RepositoryOld(new DefaultCacheManager(gconfig, config));
	}


	public static RepositoryOld testSingle(){
		Configuration config = new ConfigurationBuilder().invocationBatching().enable().build() ; // not indexable : indexing().enable().
		return new RepositoryOld(new DefaultCacheManager(config))  ;
	}
	...
	public RepositoryOld putAttribute(String key, Object value){
		attrs.put(key, value) ;
		return this ;
	}
	...
	public ReadSession login(String wsname, Analyzer queryAnalyzer) throws CorruptIndexException, IOException {
		final AbstractWorkspace workspace = loadWorkspce(wsname);
		return new ReadSessionImpl(Credential.EMANON, workspace, queryAnalyzer);
	}

	
	public ReadSession login(Credential credential, String wsname, Analyzer queryAnalyzer) throws CorruptIndexException, IOException {
		final AbstractWorkspace workspace = loadWorkspce(wsname);
	...
	private synchronized AbstractWorkspace loadWorkspce(String wsName) throws CorruptIndexException, IOException{
		if (wss.containsKey(wsName)){
			...
			OldCacheStoreConfig config = configs.get(wsName);
...
//				central = CentralConfig.oldFromDir(dir).indexConfigBuilder().build();
	...
	public Central central(String wsName){
		OldCacheStore cacheStore = (OldCacheStore) dm.getCache(wsName + ".node").getAdvancedCache().getComponentRegistry().getComponent(CacheLoaderManager.class).getCacheStore();
		return cacheStore.central() ;
	}
	
	public void defineWorkspace(String wsName, OldCacheStoreConfig config) {
		...
		defineConfig(wsName + ".node",  new ConfigurationBuilder().clustering().cacheMode(CacheMode.REPL_SYNC).invocationBatching().enable().clustering()
				...
				.addCacheLoader().cacheLoader(new OldCacheStore()).addProperty("location", config.location())
	...
	public void defineWorkspaceForTest(String wsName, OldCacheStoreConfig config) {
		configs.put(wsName, config) ;
		defineConfig(wsName + ".node",  new ConfigurationBuilder().clustering().cacheMode(CacheMode.REPL_SYNC).invocationBatching().enable().clustering()
				.sync().replTimeout(20000)
//				.eviction().maxEntries(10000)
				.loaders().preload(true).shared(false).passivation(false).addCacheLoader().cacheLoader(new OldCacheStore()).addProperty("location", config.location())
				.purgeOnStartup(false).ignoreModifications(false).fetchPersistentState(true).async().enabled(false).build()) ;
