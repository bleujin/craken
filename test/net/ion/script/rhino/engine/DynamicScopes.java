...
//			runScripts(cx, script);
	...
	static void runScripts(Context cx, Script script) {
		...
		// This must be done before scripts can be executed. The call returns a new scope that we will share.
		ScriptableObject sharedScope = cx.initStandardObjects(null, true);

		// Now we can execute the precompiled script against the scope to define x variable and f function in the shared scope.
		script.exec(cx, sharedScope);

		// Now we spawn some threads that execute a script that calls the function 'f'. The scope chain looks like this:
		// <pre>
		// ------------------ ------------------
		...
		// ------------------
		// </pre>
		// Both the shared scope and the per-thread scope have variables 'x' defined in them. If 'f' is compiled with dynamic scope enabled,
		// the 'x' from the per-thread scope will be used. Otherwise, the 'x' from the shared scope will be used. 
		// The 'x' defined in 'g' (which calls 'f') should not be seen by 'f'.
		...
		for (int i = 0; i < threadCount; i++)
			t[i].start();
		// Don't return in this thread until all the spawned threads have completed.
		for (int i = 0; i < threadCount; i++) {
			try {
				t[i].join();
	...
	static class PerThread implements Runnable {
		...
		public void run() {
			...
			try {
				...
				// We want "threadScope" to be a new top-level scope, so set its parent scope to null. 
				// This means that any variables created by assignments will be properties of "threadScope".
				threadScope.setParentScope(null);

				// Create a JavaScript property of the thread scope named 'x' and save a value for it.
