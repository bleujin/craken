...
public class CacheLoaderInterceptor extends JmxStatsCommandInterceptor {
	...
	private boolean loadIfNeeded(InvocationContext ctx, Object key, boolean isRetrieval, FlagAffectedCommand cmd) throws Throwable {
		...
		if (!isRetrieval && !ctx.isOriginLocal() && !forceLoad(key, cmd.getFlags()))
			return false;
	 ...
	 * <li>updates the 'entry' reference (an entry in the current thread's InvocationContext) with the contents of 'loadedEntry' (freshly loaded from the CacheStore) so that the loaded details will be flushed to the DataContainer when the call returns (in the LockingInterceptor, when locks are released)</li>
	 * <li>Notifies listeners</li>
	 * </ol>
	 * 
	 * @param ctx
	 *            the current invocation's context
	 * @param key
	 *            key to record
	 * @param entry
	 *            the appropriately locked entry in the caller's context
	 * @param loadedEntry
	 *            the internal entry loaded from the cache store.
	 */
